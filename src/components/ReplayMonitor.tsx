// src/components/ReplayMonitor.tsx
...
const ReplayMonitor = ({ engineData, lang: initialLang, onClose }: ReplayMonitorProps) => {
        ...
        return (
            <div className="flex justify-between items-baseline border-b border-white/[0.03] pb-[0.2vw] mb-[0.2vw]">
                ...
                <div className="flex items-baseline gap-x-1">
                    <span className={`text-[0.8vw] font-mono ${isCritical ? 'text-red-500 animate-pulse font-bold' : 'text-blue-400'}`}>
                        {val !== undefined && val !== null ? val : "---"}
                    </span>
                    <span className="text-[0.8vw] text-white/40 font-bold uppercase">{unit}</span>
                </div>
            </div>
        );
    };
    ...
    const renderStatus = (germanKey: string, val: any, activeLabel: string, type: 'OP' | 'SEL' | 'STOP' | 'ON') => {
        const label = t(germanKey);
        const isActive = String(val).trim() === activeLabel || val === "1" || val === 1;
        let colorClass = "bg-white/5 text-white/100";
        let displayVal = val ?? "---";

        if (type === 'OP') {
            colorClass = isActive ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400";
            // GANTI lang MENJADI currentLang
            displayVal = isActive ? (currentLang === 'id' ? "OPERASI" : currentLang === 'en' ? "OPERATION" : "BETRIEB") : "STOP";
        } else if (type === 'SEL') {
            colorClass = "bg-blue-500/20 text-blue-400";
            // GANTI lang MENJADI currentLang
            displayVal = isActive ? (currentLang === 'id' ? "MESIN" : currentLang === 'en' ? "MACHINE" : "MASCHINE") : (currentLang === 'id' ? "KONSOL" : currentLang === 'en' ? "CONSOLE" : "PULT");
        } else if (type === 'STOP') {
            colorClass = isActive ? "bg-red-600 text-white animate-bounce" : "bg-green-500/10 text-green-500/40";
            // GANTI lang MENJADI currentLang
            displayVal = isActive ? (currentLang === 'id' ? "DARURAT" : currentLang === 'en' ? "EMERGENCY" : "NOT-AUS") : "NORMAL";
        }
        ...
        <div className="w-full h-full bg-[#0f171d]/90 backdrop-blur-xl border border-white/10 flex flex-col overflow-hidden shadow-2xl">
            <div className="flex items-center justify-between px-[1vw] py-[0.5vw] border-b border-white/10 bg-white/5">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-[0.7vw] font-black tracking-widest text-white uppercase">Engine Replay Monitor</span>
                </div>
                <div className="flex gap-1">
                    {(['de', 'en', 'id'] as Language[]).map((l) => (
                        <button
                            key={l}
                            onClick={() => setCurrentLang(l)} // Gunakan setCurrentLang
                            className={`text-[0.5vw] px-2 py-1 font-bold uppercase transition-all ${currentLang === l ? 'bg-yellow-400 text-black' : 'bg-white/5 text-white/40 hover:bg-white/10'}`}
                        >
                            {l}
                        </button>
                    ))}
